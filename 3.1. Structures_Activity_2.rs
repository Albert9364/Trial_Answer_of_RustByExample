...
/*
添加一个 square 函数，它接受一个 Point 和一个 f32 作为参数，返回一个 Rectangle，其左上角在该点上，宽度和高度都等于 f32 参数。
*/
...
fn main() {
    let top_point: Point = Point { x: 5.2, y: 0.4 };
    let size = 2.0;

    let my_square = square(top_point, size);

    println!("生成的正方形是：{:?}", my_square);
}